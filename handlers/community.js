...
/**
* Sends a message to the Game Coordinator requesting to spectate `account_id`'s current game. 
* Provide a callback or listen for {@link module:Dota2.Dota2Client#event:spectateFriendGame|spectateFriendGame} event for Game Coordinator's response. 
* This data contains all stats shown on a player's profile page.
* Requires the GC to be {@link module:Dota2.Dota2Client#event:ready|ready}.
* @alias module:Dota2.Dota2Client#spectateFriendGame
* @param {number} account_id - Dota 2 account ID of the player who should be spectated
* @param {module:Dota2~requestCallback} [callback] - Called with `err, CMsgGCSpectateFriendGame`
*/
Dota2.Dota2Client.prototype.spectateFriendGame = function (steam_id, callback) {
    steam_id = steam_id.toString() || null;
    callback = callback || null;

    if (steam_id === null) {
        this.Logger.error("Steam ID required to spectate a match.");
        return null;
    }
    this.Logger.debug("Sending spectate request.");
    ...
    this.sendToGC(Dota2.schema.lookupEnum("EDOTAGCMsg").values.k_EMsgGCSpectateFriendGame,
        ...
        onSpectateFriendGameResponse, callback);
};
...
/**
* Emitted in response to a {@link module:Dota2.Dota2Client#spectateFriendGame|request to spectate a game}
* @event module:Dota2.Dota2Client#spectateFriendGame
* @param {number} steam_id - Steam ID of the player
* @param {module:Dota2.schema.CMsgSpectateFriendGameResponse} spectateFriendGameResponse - Spectate response.
*/
...
var onSpectateFriendGameResponse = function onSpectateFriendGameResponse(message, callback) {
    var spectateFriendGameResponse = Dota2.schema.lookupType("CMsgSpectateFriendGameResponse").decode(message);

    this.Logger.debug("Received spectate friend game response data.");
    this.emit("spectateFriendGameResponse", spectateFriendGameResponse.steam_id, spectateFriendGameResponse);
    if (callback) callback(null, spectateFriendGameResponse);
